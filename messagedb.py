...
#### GETTING IDs FROM VALUES####
    def get_phone_id(self, phone_number):
        self.cursor.execute('SELECT phone_id FROM phone_numbers WHERE phone_number = ?', (phone_number,))
    ...
    def get_system_prompt_id(self, system_prompt):
        self.cursor.execute('SELECT system_prompt_id FROM system_prompts WHERE system_prompt = ?', (system_prompt,))
        result = self.cursor.fetchone()
        if result:
            return result[0]
        else:
...
#### ADDING TO DB ####
    def add_phone_number(self, phone_number):
        ...
        if phone_id is not None:
            print(f"[DB]: Phone number '{phone_number}' already exists in the database.")
            return phone_id
        self.cursor.execute('INSERT INTO phone_numbers (phone_number) VALUES (?)', (phone_number,))
        self.conn.commit()
        phone_id = self.cursor.lastrowid
        print(f"[DB]: Phone number '{phone_number}' added successfully. Phone ID: {phone_id}")
        return phone_id
    ...
    def add_system_prompt(self, system_prompt):
        ...
        if system_prompt_id is not None:
            print(f"[DB]: System prompt '{system_prompt}' already exists in the database.")
            return system_prompt_id
        self.cursor.execute('INSERT INTO system_prompts (system_prompt) VALUES (?)', (system_prompt,))
        self.conn.commit()
        system_prompt_id = self.cursor.lastrowid
        print(f"[DB]: System prompt '{system_prompt}' added successfully. System Prompt ID: {system_prompt_id}")
        return system_prompt_id

    def add_model(self, model_name):
        model_id = self.get_model_id(model_name)
        if model_id is not None:
            print(f"[DB]: Model '{model_name}' already exists in the database.")
            return model_id
        self.cursor.execute('INSERT INTO models (model_name) VALUES (?)', (model_name,))
        self.conn.commit()
        model_id = self.cursor.lastrowid
        print(f"[DB]: Model '{model_name}' added successfully. Model ID: {model_id}")
        return model_id
    ...
    def add_settings(self, phone_number, **kwargs):
        ...
        if phone_id is not None:
            ...
            sql_question_marks = ', '.join(['?' for _ in range(len(values) + 1)])
            ...
            print(f"[DB]: Settings for phone number '{phone_number}' added successfully.")
            return settings_id
        else:
            print(f"[DB]: Phone number '{phone_number}' not found.")
...
#### UPDATING DB ####
    def update_settings_for_phone_number(self, phone_number, **kwargs):
        ...
        if phone_id is not None:
            ...
            self.conn.commit()
            print(f"[DB]: Settings for phone number '{phone_number}' updated successfully.")
        else:
            print(f"[DB]: Phone number '{phone_number}' not found.")
...
#### GETTING FROM DB ####
    def get_phone_number(self, phone_id):
        self.cursor.execute('SELECT phone_number FROM phone_numbers WHERE phone_id = ?', (phone_id,))
        result = self.cursor.fetchone()
        if result:
            return result[0]
        else:
    ...
    def get_messages_for_phone_number(self, phone_number):
        phone_id = self.get_phone_id(phone_number)
        if phone_id is not None:
            self.cursor.execute('SELECT * FROM messages WHERE from_phone_id = ? OR to_phone_id = ?', (phone_id, phone_id))
            ...
            for row in results:
                ...
                message = Message(message_id, message_sid, from_phone_number, to_phone_number, body, timestamp)
    ...
    def get_system_prompt(self, system_prompt_id):
        self.cursor.execute('SELECT system_prompt FROM system_prompts WHERE system_prompt_id = ?', (system_prompt_id,))
        result = self.cursor.fetchone()
        if result:
            return result[0]
        else:
            print(f"[DB]: System prompt with ID '{system_prompt_id}' not found.")
            return None

    def get_model(self, model_id):
        self.cursor.execute('SELECT model_name FROM models WHERE model_id = ?', (model_id,))
        result = self.cursor.fetchone()
        if result:
            return result[0]
        else:
    ...
    def get_settings_for_phone_number(self, phone_number):
        phone_id = self.get_phone_id(phone_number)
        if phone_id is not None:
            self.cursor.execute('SELECT * FROM settings WHERE phone_id = ?', (phone_id,))
            result = self.cursor.fetchone()
            if result:
                ...
                print(f"[DB]: Settings for phone number '{phone_number}' not found.")
                return None
        else:
            print(f"[DB]: Phone number '{phone_number}' not found.")
            return None
    ...
    def delete_messages_for_phone_number(self, phone_number):
        phone_id = self.get_phone_id(phone_number)
        if phone_id is not None:
            self.cursor.execute('DELETE FROM messages WHERE from_phone_id = ? OR to_phone_id = ?', (phone_id, phone_id))
            self.conn.commit()
            print(f"[DB]: Messages for phone number '{phone_number}' removed successfully.")
        else:
            print(f"[DB]: Phone number '{phone_number}' not found.")
